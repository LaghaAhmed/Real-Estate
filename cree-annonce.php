...
<!--**********************************************************BODY***************************************************************-->
  <body style='margin: auto;max-width: 1500px;min-width: 980px;width:80%'>
            ...
            for($i=1;$i<12;$i++){unset($_SESSION['f'.$i]);}
            $p=0;
            if (isset($_GET['id'])){$p=1;
              $res = "SELECT * FROM `annonce` WHERE `id`='".$_GET['id']."'" ;
...
<?php }?>
<div class;="menurecherchepro" style='position:relative;top:-440px;float:left'>
<table>
    <tr>
      <td>
          <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Adresse</H5></div><input  id="adresse" type='text' name='adresse' style='width:350px' placeholder="Positionnez le point rouge sur votre proprieté" <?php if ($p==1){echo 'value=';echo '"'.$adresse.'"';} ?>><br>
          <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Info Google</H5></div><input  id="info" type='text' name='info' style='width:350px'placeholder="Positionnez le point rouge sur votre proprieté" <?php if ($p==1) echo 'value="'.$info.'"';?>><br>
      </td>
    </tr>
    ...
    <tr  valign="top">
      <td><div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Cathegorie:</h5></div>
        ...
        <option value="Terrain" <?php if ($p==1 && $offre=='Terrain') echo 'Selected'?>>Terrain</option>
        <option value="Maison" <?php if ($p==1 && $offre=='Maison') echo 'Selected'?>>Maison</option>
        <option value="Bureau" <?php if ($p==1 && $offre=='Bureau') echo 'Selected'?>>Bureau</option>
        <option value="Parking" <?php if ($p==1 && $offre=='Parking') echo 'Selected'?>>Parking</option>          
        <option value="Fond de commerce" <?php if ($p==1 && $offre=='Fond de commerce') echo 'Selected'?>>Fond de commerce</option>
        <option value="Divers" <?php if ($p==1 && $offre=='Divers') echo 'Selected'?>>Divers</option>
        </select>
        &nbsp&nbsp
        <INPUT type="checkbox" name="type" id="v" value="1" style='display:inline-block' <?php if ($p==1 && $type=='1') echo 'Checked'?>> à vendre ou
        <INPUT type="checkbox" name="type1" id="l" value="2" style='display:inline-block' <?php if ($p==1 && $type=='2') echo 'Checked'?>> à louer
      </td>
    </tr>
    <tr>
      <td>
	 	  <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Prix</H5></div><input type='number' name='prix' placeholder="0" <?php if ($p==1) echo 'value="'.$prix.'"'?>><br>
      <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Surface</h5></div><input step='50' type='number' SIZE=2 MAXLENGTH='10' name='surface' placeholder="10m" width="200" <?php if ($p==1) echo 'value="'.$surface.'"'?>>
      <div id='avance' width="300" disabled></div>
      <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>description:</h5></div>
	   	<textarea rows="3" cols="40" maxlength='256' name='description' <?php if ($p==1) echo 'value="'.$description.'"'?>> </textarea><br>
      <div style='display:inline-block;width:13%;margin:1px;background-color:#b0c4de;'><h5>Video</H5></div><input type='text' name='video' placeholder="Coller un lien youtube ici" style='width:350px' <?php if ($p==1) echo 'value="'.$video.'"'?>><br><br>
      </td>
    </tr>
    <tr>
      <td colspan='3' align='right'>
        <input type="file" id="file" class='file' name="files[]" title="Choisir un fichier à télécharger" multiple="multiple" accept="image/*"/>
        ...
        <br />
        <progress id="prog" class='progress bar' value="0" min="0" max="100"></progress>
      </td>
    </tr>
    <tr>
  ...
  <div style="float:right;position:absolute;bottom:-70px;right:15px;clear:right">
    <p><input class="btn btn-primary btn-large"  name='deposer' type="submit" value="Déposer l'annonce"></p>
</div>
</div>
      </form>
      </div>
...
{ 
 document.getElementById('imgstore').innerHTML=
  '<div id="div'+j+'"><div style="padding-left:2px;display:inline-block;float:left;background: url('+e2.target.result +') no-repeat;-webkit-background-size:100%;-o-background-size:100%;background-size:100%;position:relative;background-size:cover;height: 148px;width: 148px;" border="2"><span id="supp" class="supp lb_close" onClick="suppimage('+j+');"></span></div></div>'+document.getElementById('imgstore').innerHTML;
  j++
}
